...
function isVersionAtLeast(actual, minimum) {
  ...
  const maxLength = Math.max(actualParts.length, minimumParts.length, 3);

  for (let i = 0; i < maxLength; i += 1) {
    const actualValue = Number.isFinite(actualParts[i]) ? actualParts[i] : 0;
...
if (isVersionAtLeast(version, '2.3.2')) {
  if (gd.GD_HEIF) {
    formats.push('Heif');
  }
  if (gd.GD_AVIF) {
...
formats.forEach(format => {
  ...
  if (!gd.Image.prototype[`save${format}`]) {
